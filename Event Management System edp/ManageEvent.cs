    ...
    public partial class ManageEvent : Form
    {
        ...
        private void EditBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Con.Open();
                    SqlCommand cmd = new SqlCommand("Update EventTb  Set EvName=@EN,EvDate=@ED,EvVenue=@EV,VenueID=@VI,EvDuration=@EDU,EvCustName=@ECN,EvCusId=@ECI,EvStatus=@ES Where EvId=@EKey", Con);
                    cmd.Parameters.AddWithValue("@EN", EvNameTb.Text);
                    cmd.Parameters.AddWithValue("@ED", EvDate.Value.Date);
                    cmd.Parameters.AddWithValue("@EV", VenueNameTb.Text);
                    cmd.Parameters.AddWithValue("@VI", EVIdCb.SelectedValue.ToString());
        ...
        private void EventDGV_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                EvNameTb.Text = row.Cells[1].Value?.ToString() ?? string.Empty;
                EvDateLb.Text = row.Cells[2].Value?.ToString() ?? string.Empty;
                EVIdCb.SelectedValue = row.Cells[3].Value?.ToString() ?? string.Empty;
                VenueNameTb.Text = row.Cells[4].Value?.ToString() ?? string.Empty;
                EvDuration.Text = row.Cells[5].Value?.ToString() ?? string.Empty;
                CustIdCb.SelectedValue = row.Cells[6].Value?.ToString() ?? string.Empty;
                CustNameTb.Text = row.Cells[7].Value?.ToString() ?? string.Empty;
                StatusCb.SelectedItem = row.Cells[8].Value?.ToString() ?? string.Empty;

                if (string.IsNullOrEmpty(EvNameTb.Text))
                {
                    Key = 0;
                }
                else
                {
                    if (int.TryParse(row.Cells[0].Value?.ToString(), out int result))
                    {
                        Key = result;
                    }
                    else
